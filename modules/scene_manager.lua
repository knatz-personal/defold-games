...
-- Called by fade.gui_script when fade completes
...
local function transition_out(cb)
  ...
  msg.post(M.transition.url, "fade_out", { callback_id = id })
end
...
local function transition_in(cb)
  ...
  local id = generate_callback_id()
  callback_registry[id] = cb
  msg.post(M.transition.url, "fade_in", { callback_id = id })
end
...
-- Handle proxy_loaded → enable + fade in
...
function M.load(scene)
  ...
  if M.current_scene and url_to_string(M.current_scene)==key then return end
...
--- Set the GUI script URL that handles fade_in/fade_out
